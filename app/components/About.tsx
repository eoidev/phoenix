...
export default async function About() {
  ...
  return (
    <section id="about" className="py-32 px-6 max-w-6xl mx-auto w-full">
      {/* 2-col, 4-row grid */}
      <div className="grid grid-cols-2 grid-rows-[repeat(4,fit-content(100%))] gap-[35px]">

        {/* Row 1 — label, col 1 only */}
        <p className="col-start-1 row-start-1 text-xs text-[#6b7280] tracking-widest uppercase">
          About
        </p>

        {/* Row 2 — heading, col 1 only */}
        <h2 className="col-start-1 row-start-2 text-4xl font-light text-[#1a1a1a] leading-snug">
        ...
        <div className="col-start-1 row-start-3 flex justify-end">
          <div className="w-[227px] h-[338px] relative overflow-hidden bg-[#f0ede8] flex items-center justify-center">
            {s?.photoUrl ? (
              <Image src={s.photoUrl} alt={s.name ?? "Photo"} fill className="object-cover" />
            ) : (
              <span className="text-[#1a1a1a]/10 text-sm">Photo</span>
            )}
            <div className="absolute inset-0 bg-[rgba(0,71,252,0.07)] mix-blend-plus-lighter pointer-events-none" />
          </div>
        </div>

        <div className="col-start-2 row-start-3 space-y-4 text-[#1a1a1a] leading-relaxed pr-16">
          {s?.bio?.length ? (
